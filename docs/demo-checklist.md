...
- [ ] **3D 자세 검증**: 미러에서 반 걸음 뒤로 물러나 같은 자세로 1턴 → 몸통 정렬
      값이 비슷하게 나오는지 (거리 불변 확인). 좌우로 체중 흔들며 1턴 → 체중 이동
      카드 등장 확인. 답변 중 손을 완전히 고정 → 경직 카드 확인
- [ ] **난입 가드**: 체험 중 다른 사람이 카메라 앞을 지나가게 하고 판정이
      튀지 않는지 (person_switch_skips가 잡는다)
