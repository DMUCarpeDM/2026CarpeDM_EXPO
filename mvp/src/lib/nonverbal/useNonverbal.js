// ⚠️ 기계 변환 사본 — 원본(단일 진실)은 poc/frontend/src/features/roleplay/useNonverbal.ts 입니다.
// 로직 수정은 반드시 poc 원본에 먼저 반영한 뒤 이 사본을 재생성하세요 (타입만 제거된 동일 코드).
import { useCallback, useEffect, useRef, useState } from 'react';
import { resolveModel, resolveWasmUrl } from './visionAssets';
import { BROW_RAISE_MIN, emptyAcc, emptyBaseline, emptyCalibration, finalizeCalibration, finalizeTurnMetrics, framesFor, gazeZoneIndex, HAND_ACTIVE_MIN, resolveGaze, resolveHeadDown, SAMPLE_MS, TIMELINE_BIN_MS, TIMELINE_MAX_BINS, trackerJumped, updateNod, verticalIrisRatio, } from './nonverbalCore';
...
export function useNonverbal(videoRef, overlayRef) {
    const [visionStatus, setVisionStatus] = useState('idle');
    const [tip, setTip] = useState(null);
    const [live, setLive] = useState(idleLive);
    const accRef = useRef(emptyAcc());
    const recentOffRef = useRef([]);
    ...
    const gazePhaseRef = useRef(null);
    // 실시간 코칭 오버레이 (F-KYJJQW) — 세션당 3회, 20초 쿨다운
    const maybeCoach = useCallback((text) => {
        const now = Date.now();
        if (tipCountRef.current >= 3 || now - lastTipAtRef.current < 20000)
            return;
    ...
    useEffect(() => {
        ...
        async function init() {
            ...
            try {
                ...
                const prevWrists = { l: null, r: null };
                let prevPerson = null;
                let prevFace = null;
                timer = setInterval(() => {
                    ...
                    try {
                        ...
                        let signedAsym = null;
                        let rollDeg = null;
                        let eyeInHeadX = null; // 홍채 수평 편향 (0=정중앙)
                        let eyeInHeadY = null; // 홍채 수직 위치 (0=위 눈꺼풀, 1=아래)
                        ...
                        if (lm) {
                            ...
                            if (lm.length > 477) {
                                ...
                                const rY = verticalIrisRatio(lm[IRIS_R], lm[EYE_LIDS_R.upper], lm[EYE_LIDS_R.lower], lm[EYE_R.inner], lm[EYE_R.outer], EYE_OPEN_MIN);
                                const lY = verticalIrisRatio(lm[IRIS_L], lm[EYE_LIDS_L.upper], lm[EYE_LIDS_L.lower], lm[EYE_L.inner], lm[EYE_L.outer], EYE_OPEN_MIN);
                        ...
                        let headGap = null;
                        let shoulderX = null;
                        let noseRelX = null; // 코 수평 위치(어깨중심 대비, 너비 정규화) — 머리 흔들림
                        ...
                        if (plm) {
                            ...
                            if (width > 0.05 && !personUnstable) {
                                ...
                                const lw = plm[15];
                                const rw = plm[16];
                                const nearFace = (w) => Math.hypot(w.x - noseP.x, w.y - noseP.y) < width * 0.6 && w.y < (ls.y + rs.y) / 2;
                                if (lw && rw) {
                                ...
                                for (const [side, wi] of [['l', 15], ['r', 16]]) {
                                    const w = wlm?.[wi];
                                    if (w && vis(w)) {
                                        handSeen = true;
                                        ...
                                        if (worldShoulder && runningRef.current) {
                                            const reach = Math.hypot(w.x - worldShoulder.x, w.y - worldShoulder.y, w.z - worldShoulder.z);
                                            if (reach < 1.2) {
                        ...
                        if (calibratingRef.current) {
                            ...
                            if (signedAsym !== null)
                                cal.asym.push(signedAsym);
                            if (tiltRaw !== null)
                                cal.tilt.push(tiltRaw);
                            if (headGap !== null)
                                cal.headGap.push(headGap);
                            if (rollDeg !== null)
                                cal.roll.push(rollDeg);
                            if (eyeInHeadX !== null)
                                cal.eyeX.push(eyeInHeadX);
                            if (eyeInHeadY !== null)
                                cal.eyeY.push(eyeInHeadY);
                            if (shoulderWidth !== null)
                                cal.width.push(shoulderWidth);
                            // 깜빡임 기저선(동역학): 얼굴이 추적된 프레임에서만 세어 비율 왜곡 방지
                            if (lm) {
                                cal.blinkFrames += 1;
                                if (blink && !cal.blinkOn)
                                    cal.blinks += 1;
                                cal.blinkOn = blink;
                            }
                        }
                        // ---- 기준 대비 판정: 시선 = 머리 자세 + 홍채 보상 (nonverbalCore) ----
                        const gaze = resolveGaze({ signedAsym, eyeInHeadX, eyeInHeadY, eyeDown, eyeUp }, base);
                        const { front, offDir, gazeX, eyeYDelta } = gaze;
                        if (runningRef.current && gaze.irisVUsed)
                            acc.irisVFrames += 1;
                        ...
                        if (runningRef.current && faceUnstable)
                            acc.guardFrames += 1;
                        if (runningRef.current && (lm || plm) && !faceUnstable) {
                            ...
                            if (front) {
                                acc.frontFrames += 1;
                                if (acc.curOffStreak > 0)
                                    acc.offStreaks.push(acc.curOffStreak); // 회복 완료
                                acc.curOffStreak = 0;
                            ...
                            if (phase === 'listening') {
                                ...
                                if (shoulderWidth !== null)
                                    acc.listenWidths.push(shoulderWidth);
                                // 끄덕임 근사 (nonverbalCore.updateNod) — 긍정 신호 전용
                                if (headGap !== null)
                                    updateNod(acc, headGap);
                            }
                            ...
                            if (acc.turnStartedAt) {
                                const binIdx = Math.min(TIMELINE_MAX_BINS - 1, Math.floor((Date.now() - acc.turnStartedAt) / TIMELINE_BIN_MS));
                            ...
                            if (shoulderWidth !== null)
                                acc.shoulderWidths.push(shoulderWidth);
                            if (worldUsed)
                                acc.worldFrames += 1;
                            if (handSeen)
                                acc.handSeenFrames += 1;
                            if (lowerVisible)
                                acc.lowerVisFrames += 1;
                            if (hipX !== null)
                                acc.hipXs.push(hipX);
                            acc.lastFront = front;
                            if (blink && !acc.blinkActive)
                                acc.blinkCount += 1;
                            acc.blinkActive = blink;
                            if (smile) {
                                acc.smileFrames += 1;
                                if (duchenne)
                                    acc.duchenneFrames += 1;
                            }
                            if (mouthPress)
                                acc.mouthPressFrames += 1;
                            if (browDown)
                                acc.browDownFrames += 1;
                            if (browRaise)
                                acc.browRaiseFrames += 1; // 표정 생동감 (관찰 지표)
                            ...
                            if (handFace)
                                acc.handFaceFrames += 1;
                            if (armCross)
                                acc.armCrossFrames += 1;
                            if (rollAdj !== null)
                                acc.rollSamples.push(rollAdj);
    ...
    const setGazePhase = useCallback((phase) => {
