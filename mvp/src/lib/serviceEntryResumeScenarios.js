  ...
  for (const resume of ["error", "timeout", "missing", "unknown"]) {
    observations.push(await withPage(pageHarness, {
      ...
      if (resume === "timeout") {
        ...
        assert.equal(await page.locator(".service-mode-page").count(), 0, "service has no pre-timeout first paint");
      ...
      await capture(page, join(runDir, `fallback-${resume}-service.png`), ".service-mode-page");
