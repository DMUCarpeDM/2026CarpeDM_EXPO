  ...
  return (
    <section className="page home-page">
      ...
      <section className="home-fit-section" aria-label="4-Fit 코칭 분석 소개">
        <div className="home-section-head"><span>4-FIT 코칭 분석</span><h2>4가지 신호로<br />말하는 방식을 살펴봐요</h2><p>응답·목소리·표정·자세를 함께 살펴보고, 다음 연습에 바로 쓸 수 있는 팁을 알려드려요.</p></div>
...
function HomeFitCard({ image, icon, label, short, score, tone, text }) {
  return <article className={`home-fit-card ${tone}`}><span aria-label={label}>{image ? <img src={image} alt="" aria-hidden="true" /> : <IconGlyph icon={icon} size={40} />}<b>{short}</b><small>({label.split("(")[1]}</small></span><strong>{score}<small>/100</small></strong><b>{score >= 80 ? "좋아요" : "보통이에요"}</b><MiniLineChart tone={tone} /><p>{text}</p></article>;
}
...
function MiniLineChart({ tone }) {
  const paths = {
    ...
    expression: "M3 42 L18 36 L35 24 L52 31 L69 26 L86 12 L107 12 L128 4",
    posture: "M3 44 L20 34 L36 23 L52 31 L69 25 L87 18 L106 6 L128 4",
  };
  const colors = { response: "#0969f7", voice: "#6a4df5", expression: "#7c3aed", posture: "#ff8a00" };
...
function HomeRadarChart({ fits }) {
  ...
  const labels = [[100, 12, "응답"], [190, 104, "목소리"], [100, 195, "표정"], [10, 104, "자세"]];
  ...
  return <svg className="home-radar-chart" viewBox="0 0 200 200" role="img" aria-label="응답 82점, 목소리 76점, 표정 84점, 자세 71점의 4-Fit 종합 점수">
