...
import { KioskIssuePage } from "./pages/KioskIssuePage";
...
// 발급 키오스크 모드 (E-17) — ?kiosk=issue로 켠다. 발급 PC는 이 화면만 전용으로 띄우므로
// 전시 흐름(홈·연습·리포트)과 유휴 리셋·세션 재개 로직에서 완전히 분리한다.
const KIOSK_ISSUE_MODE = new URLSearchParams(window.location.search).get("kiosk") === "issue";
...
const DEMO_REPORT = {
  ...
  // ---- B2B 확장 데모 (E-21/F-4) — 등급·영수증 QR·Before→After 코칭 카드 미리보기용
  grade: "우수",
...
// 감정 온도 게이지 데모 (S-B2B-EMOTION) — ?demo=practice에서 게이지·완화 연출을 미리 본다.
const DEMO_TURN_SIGNALS = { case: "covered", coverage: 0.72, emotion: { state: "displeased", label: "불만", temperature: 48, eased: true } };
...
export default function App() {
  ...
  // ---- 미러 NFC 즉시 시작 (E-18) — 태그로 해석된 카드 정보. 설정 단계를 건너뛰고
  // 동의 화면(preview)만 거쳐 시작한다. 수동 폴백(E-20)은 uid 없이 직무만 채운다.
  const [nfcCard, setNfcCard] = useState(null); // {uid, jobRole, scenarioSlug, jobRoleLabel}
  ...
  // NFC 카드 시작은 카드에 귀속된 시나리오가 최우선이다 — 직전 관람객의 설정 선택이 남아 있어도 덮는다.
  ...
  const navigate = (target) => {
    // 홈 복귀·일반 설정 흐름 진입 시 NFC 시작 정보를 비운다 — 카드 흐름이 남아
    // 다음 관람객의 일반 선택 흐름을 오염시키지 않게 한다.
  ...
  useEffect(() => {
    if (KIOSK_ISSUE_MODE) return; // 발급 키오스크 PC에서는 데모 진입을 무시한다
    const demo = new URLSearchParams(window.location.search).get("demo");
    if (!demo) return;
    setReport(DEMO_REPORT);
    setHistory(DEMO_HISTORY);
    if (demo === "practice") {
      setSession(DEMO_SESSION); setTurn(DEMO_TURN); setTurnHistory(DEMO_TURN_HISTORY); setTurnSignals(DEMO_TURN_SIGNALS); setActive("practice");
  ...
  useEffect(() => {
    if (KIOSK_ISSUE_MODE) return; // 발급 키오스크 PC에서는 연습 세션을 재개하지 않는다
  ...
  // ---- 미러 NFC 즉시 시작 (E-18 / S-B2B-105): 태그 → resolve → 동의 화면으로 직행.
  // 동의는 건너뛰지 않는다(개인정보 원칙) — 설정 3단계만 카드가 대신한다.
  const handleMirrorTap = async (tap) => {
    ...
    try {
      ...
      // 미등록·폐기 카드(404)·일시 오류 — 수동 직무 선택 폴백 (E-20)
      setNfcFallback(true);
    } finally {
      nfcResolvingRef.current = false;
    }
  };
  // 홈(대기) 화면에 있을 때만 미러 리더를 폴링한다 — 다른 화면·키오스크 모드에선 중지.
  useNfcTap({ reader: "mirror", enabled: active === "home" && !KIOSK_ISSUE_MODE, onTap: handleMirrorTap });
  // 수동 폴백 — 카드 없이 직무만으로 같은 흐름(동의 → 연습)을 잇는다.
  ...
  const startPractice = async () => {
    ...
    try {
      ...
      try {
        ...
        // 카메라·마이크가 전면 거부돼도 체험을 막지 않는다(운영 약속: "체험은 카메라
        // 없이도 진행됨"). 연습 화면이 권한 배너·재연결 버튼·텍스트 입력으로 이어받고,
        // 권한이 살아나면 다음 요청부터 분석이 자동 복구된다. 시선·자세·음성 지표만 비운다.
        console.warn("[media] 카메라·마이크 없이 시작:", mediaError?.message || mediaError);
      }
      // NFC 시작이면 카드의 직무·시나리오를 세션에 스탬프한다 (에피소드는 서버 기본 선택).
  ...
  // NFC 시작의 미리보기는 카드 직무에 맞는 상대 이미지를 쓰되, 역할·성격 문구는
  // 시나리오의 실제 등장인물(카페 온도 고객 등) 데이터로 채운다.
  const nfcRole = nfcCard ? findJobRole(nfcCard.jobRole) : null;
  const previewCounterpartProfile = nfcCard
    ? { image: (counterpartProfiles.find((item) => item.id === nfcRole?.counterpartProfileId) || counterpartProfiles[3]).image }
    : counterpartProfiles.find((item) => item.id === counterpartProfile) || counterpartProfiles[1];

  // 발급 키오스크 PC 전용 화면 (E-17) — 전시 셸(TopNav·AttractLoop·유휴 리셋)과 분리해 렌더한다.
  if (KIOSK_ISSUE_MODE) return <KioskIssuePage />;
  ...
  return <main className={`app-shell ${chromeless ? "chromeless" : ""} ${active === "practice" ? "practice-mode" : ""}`}>
    ...
    <div className="screen-frame">
      ...
      {active === "preview" && <PreviewPage onNext={startPractice} starting={starting} scenario={previewScenario} selectedEpisode={previewEpisode} counterpartProfile={previewCounterpartProfile} difficulty={difficulties.find((item) => item.id === difficulty) || difficulties[0]} aiHealth={aiHealth} consented={consented} onConsent={setConsented} error={apiError} permissionState={permissionState} mode={mode} />}
