...
AI 질문은 PoC 서버의 GPT-4o가 선택한 장면을 기준으로 만듭니다. 질문이 표시되는 동안 ElevenLabs 음성과 상대의 `speaking` 영상이 재생되고, 대화 로그에는 현재 AI 질문만 표시됩니다. ElevenLabs를 쓸 수 없으면 브라우저 음성으로 전환합니다.

## 실행

### 1. PoC 분석 서버

Python 3.12와 서버 환경 변수를 준비한 뒤 다른 터미널에서 실행합니다.

```bash
cd ../poc/backend
cp .env.example .env  # 최초 1회: OpenAI·ElevenLabs 값을 채움
.venv/bin/uvicorn app.main:app --host 127.0.0.1 --port 8001
```

`MIRROR_TING_OPENAI_API_KEY`는 실제 시뮬레이션의 필수 대화 키입니다. `MIRROR_TING_ELEVENLABS_API_KEY`와 `MIRROR_TING_ELEVENLABS_VOICE_ID`를 채우면 AI 상대 음성도 ElevenLabs로 재생됩니다. 서버가 `http://127.0.0.1:8001/api/health`에서 `dialogue_ready: true`, `tts_ready: true`를 반환하는지 확인하세요.
...
- OpenAI 키는 대화 생성, ElevenLabs 키와 Voice ID는 AI 상대 음성 생성에만 서버에서 사용합니다. 브라우저 코드와 Git에는 넣지 않습니다.
