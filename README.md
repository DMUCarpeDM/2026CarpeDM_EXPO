...
| Eye-Fit | **홍채 기반 시선 추정(수평+수직, 머리 자세 보상 — 고개를 돌려도 눈이 카메라를 보면 정면)** · 상하 판정 합의/구제(블렌드셰이프×홍채, 깜빡임 게이트) · 듣기/말하기 응시 분리 · 응시 리듬 · 개시 회피 관용 · **3×3 시선 지도(리포트 히트맵)** · **깜빡임 급증 순간 감지** · 미세 안정성·회복 시간 | MediaPipe Face Landmarker 478 랜드마크 (브라우저) |
| Posture-Fit | **3D 월드 랜드마크(미터 단위) — 카메라 거리와 무관한 판정**: 몸통 수직 정렬(개인 중립 자세 보정)·좌우 체중 이동(cm)·제스처 에너지(경직/과다)·후반 붕괴 추세(3D) · 어깨 기울기·고개 숙임·흔들림 · 무의식 습관(손-얼굴·팔짱) · **가시성 신뢰 게이트 + 다인 난입 가드 + 전신/상반신 측정 범위 자가 보고** | MediaPipe Pose Landmarker worldLandmarks (브라우저) |
| **표정** (관찰) | **진정성 미소(Duchenne 근사 — 입만 웃는 미소와 눈까지 웃는 미소 구분)** · 미소 타이밍(압박 교차)×진정성 결합 판정 · **표정 복구 시간**(긴장 에피소드 지속 — 압박 내성 프로브 연동) · 입술 압축·찡그림 | Face Landmarker blendshape |
