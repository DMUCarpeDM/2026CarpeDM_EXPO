...
| 지표 | 측정 항목 | 구현 |
|------|----------|------|
| Response-Fit | 체크리스트 커버리지 · 위험/권장 표현 · 종결어미 격식 비율 · **담화 구조**(결론 선행 BLUF·근거 연결·기한 있는 약속·책임 문형·헤지 밀도·질문 정합성·만연체) | kiwipiepy 형태소 분석 + 전문가 설계 체크리스트 |
| Voice-Fit | 말속도(+전/후반 추세) · 침묵 구조(개시 지연·긴 침묵) · 성량 안정성/후반 변화 · 억양 다이내믹스(자기상관 F0) · **피치 떨림(jitter)·성량 떨림(shimmer)·주기성 강도(HNR 근사)·문말 억양 기울기** | numpy DSP (서버) — 합성 신호 대조 테스트로 검증 |
| Eye-Fit | 정면 응시 비율 · 이탈 방향 분포 · 최장 연속 이탈 · 깜빡임 빈도 · **시선 미세 안정성 · 이탈 후 회복 시간** | MediaPipe Face Landmarker (브라우저) |
...
| **심층 교차 분석** | **압박 내성 프로파일**(압박 vs 평상 턴의 시선·깜빡임·표정·떨림·침묵 델타 → 침착형/회복형/동요형) · **적응 곡선**(전/후반 추세) · **말의 구조 종합** | 리포트 파이프라인 — 관찰·해석·처방 3단 코칭 |
