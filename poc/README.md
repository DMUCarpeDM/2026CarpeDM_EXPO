...
표정·자세를 분석해 4가지 지표(4-Fit)와 근거 기반 코칭 리포트를 제공한다.
(시선은 점수 축이 아니라 실시간 넛지·리포트 관찰로 쓰는 보조 신호다.)
...
| Expression-Fit ⚠️참고용 | **표정 표현력(경직↔생동 — 감정 분류 아님, 행동 신호만)** · **눈썹 생동감**(무표정↔풍부) · **진정성 미소 근사(Duchenne — 입만 웃는 미소 vs 눈까지 웃는 미소)** · **긴장(입술 압축)** · **표정 복구 시간** · 미소 비율·타이밍(압박 질문 교차 판정). **4축 중 최저 가중치**, α 검증(10월) 전 참고 지표(설계서 철칙) | MediaPipe Face Landmarker blendshape (브라우저) |
...
| 시선 (보조 신호 · 점수 아님) | **홍채 기반 시선 추정 — 수평(머리 자세 보상) + 수직(눈꺼풀 사이 홍채 위치)** · 듣기/말하기 응시 분리 · **3×3 시선 존 히트맵** · **깜빡임 동역학**(개인 기저선 대비 급증) · 이탈 방향·최장 이탈 · 회복 시간 · **얼굴 다인 가드**. 실시간 "시선 유지" 넛지 + 리포트 관찰로만 사용(4-Fit 총점 제외) | MediaPipe Face Landmarker 478 (브라우저) |
...
│ MediaPipe 표정·자세 (온디바이스)│ ◄─────── │ 코칭 리포트 생성 (처방 템플릿)    │
...
```
docs/README.md         문서 색인 (기획·설계·검증·전시 문서 지도)
docs/prd.json          기획서 원본 (요구사항 9 · 기능 31 · 스펙 78)
docs/expo-2026-masterplan.md   상위 기획서 (PM 체크리스트) · 4fit-scoring-design.md · hardware-spec-2026.md
backend/
  app/core/            설정 · DB · JWT
