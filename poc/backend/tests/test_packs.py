...
def test_seed_packs_idempotent_and_active():
    ...
    try:
        ...
        assert scenario.characters, "역할극에 사용할 상대 페르소나가 있어야 한다"
        assert all(character.get("role") for character in scenario.characters)
...
def test_pack_scenarios_visible_in_listing():
    ...
    assert {"ondo-cafe-crew", "ondo-cs-agent", "release-schedule-alignment", "workplace-conversation"} <= slugs
