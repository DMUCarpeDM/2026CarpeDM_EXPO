...
export function postureFeatures(landmarks: {x:number; y:number; z:number}[] | undefined) {
  ...
  const center = (['x','y','z'] as const).map(k => (points[1]![k]+points[2]![k])/2);
  const width = Math.hypot(...(['x','y','z'] as const).map(k => points[1]![k]-points[2]![k]));
  if (width < .005) return null;
  return points.flatMap(p => (['x','y','z'] as const).map((k,i) => (p![k]-center[i])/width));
}
